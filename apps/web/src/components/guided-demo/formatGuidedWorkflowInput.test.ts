...
import { formatGuidedWorkflowInputFromRecords } from "./formatGuidedWorkflowInput";

type WorkflowInputRecord = Parameters<
...
function buildRecord(
  ...
  return {
    ...
    confidence: 0.88,
    reviewNeeded: true,
    sourceText: "PING,G430,driver,R,8.0 Average,$180,207",
    normalizedText:
...
describe("formatGuidedWorkflowInputFromRecords", () => {
  it("hands canonical fields to Step 3 while preserving original source evidence", () => {
    const csvSourceText = "PING,G430,driver,R,8.0 Average,$180,207";
    const priorReviewSourceText =
      "PING G425 4-PW, shaft firm, condition 8.0 Average, trade value $210, store 207.";

    const output = formatGuidedWorkflowInputFromRecords(
      [
        buildRecord({
        ...
        buildRecord({
          ...
          confidence: 0.72,
          sourceText: priorReviewSourceText,
          normalizedText:
    ...
    expect(output).toContain("shaft flex REGULAR");
    expect(output).toContain("source evidence: " + priorReviewSourceText);
    expect(output).not.toBe([csvSourceText, priorReviewSourceText].join("\n"));
  });
});
