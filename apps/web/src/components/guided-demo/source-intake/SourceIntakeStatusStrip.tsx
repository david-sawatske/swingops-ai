...
export function SourceIntakeStatusStrip({
  ...
  return (
    <div className="guided-workflow-intake-toolbar">
      <section
        className="multi-source-intake-status-strip"
        aria-label="Source intake status"
      >
        ...
        <div className="multi-source-intake-status-line">
          ...
          <span>
            <strong>{selectedTypeCount}</strong> source type
            {selectedTypeCount === 1 ? "" : "s"}
          </span>
          <span>
            <strong>{readySourceCount}</strong> ready source
            {readySourceCount === 1 ? "" : "s"}
          </span>
          <span>
            Normalization:{" "}
            <strong>{isNormalizationComplete ? "complete" : "not run"}</strong>
          </span>
        </div>
      </section>
    </div>
