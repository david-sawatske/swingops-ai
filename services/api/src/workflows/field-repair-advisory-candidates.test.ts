...
import { buildPriorReviewFieldRepairAdvisoryCandidates } from "./field-repair-advisory-candidates.js";
...
function buildPriorSuggestion(
  ...
  return {
    ...
    evidenceText: "Reviewer confirmed shaft firm means Stiff.",
    confidence: 0.94,
    strength: "STRONG",
    confidenceImpact: "Require reviewer action before changing the record.",
    reasonCodes: ["RAW_TEXT_MATCH", "SHAFT_FLEX_CONTEXT_MATCH"],
    summary: "Prior review suggestion: shaftFlex = Stiff.",
    whySuggestionExists:
...
describe("buildPriorReviewFieldRepairAdvisoryCandidates", () => {
  it("builds typed review-required candidates across every supported repair field", () => {
    const sourceText = "Cally Rogue ST mx 5-PW shaft firm cond avg value $145";

    const candidates = buildPriorReviewFieldRepairAdvisoryCandidates({
      recordId: "record-1",
      sourceText,
      missingFields: [
        "brand",
      ...
      priorReviewSuggestions: [
        ...
        buildPriorSuggestion({
          ...
          previousCorrectedValue: "Rogue ST Max",
          sourceLearningEventId: "product-line-event",
        }),
        buildPriorSuggestion({
          fieldName: "category",
          rawTextMatch: "5-PW",
        ...
        buildPriorSuggestion({
          ...
          suggestedValue: "Stiff",
          previousCorrectedValue: "Stiff",
          sourceLearningEventId: "shaft-flex-event",
        }),
        buildPriorSuggestion({
          fieldName: "conditionGrade",
          rawTextMatch: "cond avg",
          suggestedValue: "8.0 Average",
          previousCorrectedValue: "8.0 Average",
          sourceLearningEventId: "condition-event",
        }),
        buildPriorSuggestion({
          ...
          sourceLearningEventId: "trade-value-event",
        }),
      ],
    });
    ...
    expect(
      candidates.map((candidate) => ({
        ...
        candidateValue: candidate.suggestion.candidateValue,
        sourcePhrase: candidate.suggestion.sourcePhrase,
        reviewRequired: candidate.suggestion.reviewRequired,
  ...
  it("withholds candidates that are not strong, missing, exact, applicable, or authoritative", () => {
    const baseInput = {
      recordId: "record-1",
      sourceText: "PING G425 putter shaft firm condition avg",
      missingFields: ["shaftFlex", "conditionGrade"],
      fieldApplicability: {
        shaftFlex: "NOT_APPLICABLE" as const,
      },
      productResolutionStatus: "MATCHED" as const,
      sourceEvidenceId: "record-1:prior-review",
    };
    ...
    const candidates = buildPriorReviewFieldRepairAdvisoryCandidates({
      ...
      priorReviewSuggestions: [
        ...
        buildPriorSuggestion({
          ...
          strength: "MEDIUM",
          confidence: 0.78,
          sourceLearningEventId: "medium-event",
        }),
        buildPriorSuggestion({
          fieldName: "brand",
          rawTextMatch: "PING",
          suggestedValue: "PING",
          sourceLearningEventId: "matched-brand-event",
        }),
        buildPriorSuggestion({
          fieldName: "conditionGrade",
          rawTextMatch: "not in source",
          suggestedValue: "8.0 Average",
          sourceLearningEventId: "missing-phrase-event",
        }),
        buildPriorSuggestion({
          fieldName: "storeId",
          rawTextMatch: "PING",
          ...
          sourceLearningEventId: "unsupported-field-event",
        }),
      ],
    });
  ...
  it("withholds all candidates for a field when strong prior corrections conflict", () => {
    const candidates = buildPriorReviewFieldRepairAdvisoryCandidates({
      recordId: "record-1",
      sourceText: "PING G425 shaft firm or flex smooth",
  ...
  it("keeps only the strongest duplicate candidate for the same field and value", () => {
    const candidates = buildPriorReviewFieldRepairAdvisoryCandidates({
      recordId: "record-1",
      sourceText: "PING G425 shaft firm flex firm",
      missingFields: ["shaftFlex"],
      fieldApplicability: {
        shaftFlex: "REQUIRED",
      },
      ...
      priorReviewSuggestions: [
        buildPriorSuggestion({
          rawTextMatch: "shaft firm",
          confidence: 0.94,
          sourceLearningEventId: "stronger-event",
        }),
        buildPriorSuggestion({
          rawTextMatch: "flex firm",
          confidence: 0.9,
          sourceLearningEventId: "weaker-event",
        }),
      ],
    });
    ...
    expect(candidates[0]).toMatchObject({
      candidateId: "prior-review:stronger-event:shaftFlex",
      suggestion: {
        fieldName: "shaftFlex",
        candidateValue: "STIFF",
        sourcePhrase: "shaft firm",
  ...
  it("preserves product candidate comparison precedence for ambiguous records", () => {
    expect(
      buildPriorReviewFieldRepairAdvisoryCandidates({
        recordId: "record-1",
        sourceText: "Titleist TSR shaft firm generation unclear",
        missingFields: ["shaftFlex"],
        fieldApplicability: {
          shaftFlex: "REQUIRED",
        },
        ...
        sourceEvidenceId: "record-1:prior-review",
        priorReviewSuggestions: [buildPriorSuggestion()],
      }),
    ).toEqual([]);
  });
});
