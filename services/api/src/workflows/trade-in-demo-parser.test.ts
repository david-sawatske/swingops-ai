...
describe("parseTradeInDemoText", () => {
  ...
  it("preserves source-supported product text when normalized handoff reference resolution is unresolved", () => {
    ...
    expect(parsedItems[0]?.missingFields).not.toEqual(
    ...
    expect(parsedItems[1]?.missingFields).not.toEqual(
      expect.arrayContaining(["productLine", "shaftFlex"]),
    );
    ...
    expect(parsedItems[2]?.missingFields).toEqual(
  ...
  it("does not treat a category-only normalized identity as product text", () => {
    ...
    expect(parsedItems[0]?.missingFields).toContain("productLine");
    expect(parsedItems[0]?.parserEvidence?.productLine).toBeUndefined();
  });
  ...
  it("preserves normalized-intake records and record-local stores through the guarded parser", () => {
    ...
    const parsedItems = parseTradeInDemoText(sourceLines.join("\n"));

    expect(parsedItems).toHaveLength(4);
    expect(parsedItems.map((item) => item.rawLine)).toEqual(sourceLines);
    ...
    const ambiguousTitleist = parsedItems[1];
    ...
    if (
      !ambiguousTitleist ||
      ambiguousTitleist.productResolution.status !== "AMBIGUOUS"
    ) {
      throw new Error("Expected the Titleist record to remain ambiguous.");
    }

    const candidateProductLines =
      ambiguousTitleist.productResolution.candidates.map(
        (candidate) => candidate.productLine,
      );

    expect(candidateProductLines).toEqual(
      expect.arrayContaining(["TSR2", "TSR3"]),
    );
    expect(candidateProductLines).toContain(ambiguousTitleist.productLine);
    expect(ambiguousTitleist.uncertaintyNotes).toContain("model uncertain");
    ...
    expect(parsedItems[3]?.missingFields).not.toContain("shaftFlex");
  });
  ...
  it("normalizes guarded workflow value, flex, and condition shorthand variants", () => {
    ...
    const parsedItems = parseTradeInDemoText(
      cases.map((testCase) => testCase.raw).join("\n"),
    );
  ...
  it("attaches authoritative and ambiguous product resolution evidence", () => {
    ...
    expect(parsedItems[1]?.uncertaintyNotes).toContain("model uncertain");
    ...
    expect(parsedItems[2]?.missingFields).toContain("productLine");
  });
  ...
  it("recognizes an injected product without adding parser product rules", () => {
    ...
    expect(parsedItems[0]).toMatchObject({
      ...
      parserEvidence: {
        productLine: {
          value: "Nova X",
          sourceText: "nx prototype driver",
        },
      },
    });
  });
  ...
  it("keeps putter shaft flex not applicable after reference resolution", () => {
    ...
    expect(parsedItems[0]?.missingFields).not.toContain("shaftFlex");
    expect(parsedItems[0]?.uncertaintyNotes).not.toContain(
      "product reference unresolved",
    );
  });
  ...
  it("preserves canonical shaft and condition when quoted LOG evidence only marks generation unclear", () => {
    ...
    expect(parsedItems[0]?.missingFields).not.toContain("shaftFlex");
    expect(parsedItems[0]?.missingFields).not.toContain("conditionNotes");
  });
});
