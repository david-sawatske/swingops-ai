...
function parseRecord(rawContent: string, provider?: ProductReferenceProvider) {
  return buildRecord(
    {
      id: "matrix_source",
...
function parseCsvRecord(rawContent: string, fragment: string, index = 0) {
  return buildRecord(
    {
...
describe("multi-source intake parser normalization matrix", () => {
  ...
  it("preserves meaningful incomplete records while excluding source noise", () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      const fragments = splitSourceIntoRecordFragments({
        id: "segmentation_matrix_" + String(index + 1),
        sourceType: testCase.sourceType,
        sourceName: testCase.sourceName,
        rawContent: testCase.lines.join("\n"),
      });

      expect(fragments, testCase.sourceName).toEqual(
  ...
  it("maps positional CSV value and store columns by header", () => {
    ...
    expect(titleistRecord.missingFields).not.toContain("tradeInValue");
  ...
  it("uses CSV headers as context for source-supported product and flex values", () => {
    ...
    const records = fragments.map((fragment, index) =>
      parseCsvRecord(rawContent, fragment, index),
    );
    ...
    expect(stealth?.parserEvidence?.shaftFlex).toEqual({
      value: "STIFF",
      sourceText: "S",
    });
    expect(stealth?.missingFields).not.toContain("shaftFlex");
    ...
    expect(g430?.parserEvidence?.productLine).toEqual({
      value: "G430",
      sourceText: "G430",
    });
    expect(g430?.parserEvidence?.shaftFlex).toEqual({
      value: "REGULAR",
      sourceText: "R",
    });
    expect(g430?.missingFields).not.toEqual(
      expect.arrayContaining(["productLine", "shaftFlex"]),
    );
    ...
    expect(odyssey?.missingFields).not.toContain("shaftFlex");
    ...
    expect(g425?.parserEvidence?.shaftFlex).toEqual({
      value: "TOUR_X_STIFF",
      sourceText: "TX",
    });
    expect(g425?.missingFields).not.toContain("shaftFlex");
  });
  ...
  it("preserves meaningful unresolved product text for non-CSV records without preserving placeholders", () => {
    ...
    const prototype = buildRecord(freeTextSource, freeTextSource.rawContent, 0);
    ...
    expect(g430.missingFields).not.toContain("productLine");
    ...
    expect(prototype.missingFields).not.toContain("productLine");
    ...
    expect(mysteryClub.missingFields).toEqual(
  ...
  it("attaches matched, ambiguous, and unresolved product resolution", () => {
    ...
    expect(unresolved.missingFields).not.toContain("productLine");
  ...
  it("recognizes an injected product across every supported source structure", () => {
    ...
    for (const sourceInput of sources) {
      const fragments = splitSourceIntoRecordFragments(sourceInput, provider);

      expect(fragments).toHaveLength(1);
      expect(fragments[0]).toContain("nx prototype driver");

      const record = buildRecord(sourceInput, fragments[0]!, 0, provider);
  ...
  it("prioritizes explicit LOG identity keys over timestamp and whole-line guesses", () => {
    ...
    const record = buildRecord(
      {
        ...
        sourceType: "LOG",
        sourceName: "Structured LOG identity",
        rawContent,
      },
      rawContent,
      0,
    ...
    expect(record.missingFields).toEqual(
      expect.arrayContaining(["conditionGrade", "tradeInValue"]),
    );
    expect(record.missingFields).not.toEqual(
      expect.arrayContaining(["brand", "productLine", "category", "shaftFlex"]),
    );
    expect(record.parserEvidence).toMatchObject({
      brand: {
        value: "PING",
      ...
      shaftFlex: {
        value: "REGULAR",
        sourceText: expect.stringMatching(/regular/i),
      },
    });
  });
  ...
  it("does not interpret operational timestamp numbers as wedge identity", () => {
    ...
    const record = buildRecord(
      {
        ...
        sourceType: "LOG",
        sourceName: "Timestamp category safety",
        rawContent,
      },
      rawContent,
      0,
    ...
    expect(record.category).not.toBe("WEDGE");
    expect(record.parserEvidence?.category).toBeUndefined();
  });
  ...
  it("retains existing free-text wedge recognition outside operational timestamps", () => {
    ...
    expect(namedWedge.category).toBe("WEDGE");
    expect(loftWedge.category).toBe("WEDGE");
  });
  ...
  it("preserves supported shaft and condition fields when only product generation is unclear", () => {
    ...
    const record = buildRecord(source, fragments[0]!, 0);
