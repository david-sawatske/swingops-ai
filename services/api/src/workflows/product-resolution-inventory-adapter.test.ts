...
describe("buildInventoryLookupFromProductResolution", () => {
  it("uses the resolver stable product ID and SKU for an authoritative match", () => {
    ...
    const lookup = buildInventoryLookupFromProductResolution({
      resolution,
      fallback: {
        brand: "TaylorMade",
  ...
  it("keeps ambiguous candidates visible without assigning an authoritative SKU", () => {
    const resolution = resolveProductReference({
      brand: "Titleist",
    ...
    expect(lookup.similarProducts.length).toBeGreaterThanOrEqual(2);
    expect(lookup.matchReasons).toContain(
      "Product reference candidates require human confirmation before inventory or valuation use.",
    );
  });

  it("does not invent identity for an unresolved product", () => {
    const resolution = resolveProductReference({
      brand: "Titleist",
      category: "driver",
