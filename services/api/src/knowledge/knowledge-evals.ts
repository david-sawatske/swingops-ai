...
const EVAL_CASES: KnowledgeRetrievalEvalCase[] = [
  ...
  },
  {
    ...
    expectedTerms: ["jpx921", "hot metal"],
  },
  {
  ...
  },
  {
    ...
    expectedTerms: ["white hot versa", "putter"],
  },
  {
    ...
    expectedCategory: "PUTTER",
    expectedTerms: ["newport 2", "putter"],
  },
  {
  ...
  },
  {
    ...
    expectedCategory: "PUTTER",
    expectedTerms: ["spider tour", "mallet"],
  },
];
...
function evaluateCase(
  ...
  if (
    topResult &&
    normalize(topResult.brand) !== normalize(evalCase.expectedBrand)
  ) {
    failures.push(
      `Expected brand ${evalCase.expectedBrand}, received ${topResult.brand ?? "null"}.`,
    );
  }
  ...
  if (
    topResult &&
    normalize(topResult.category) !== normalize(evalCase.expectedCategory)
  ) {
    failures.push(
      `Expected category ${evalCase.expectedCategory}, received ${topResult.category ?? "null"}.`,
    );
  }
  ...
  if (
    ...
    failures.push(
      `Expected chunk type ${evalCase.expectedChunkType}, received ${topResult.chunkType}.`,
    );
  }
  ...
  if (missingTerms.length > 0) {
    failures.push(
      `Missing expected matched terms: ${missingTerms.join(", ")}.`,
    );
  }

  const citationPresent = Boolean(
    topResult?.citation.sourceName && topResult.citation.documentTitle,
  );
...
export async function runKnowledgeRetrievalEvals(
  input: {
    sourceName?: string;
  } = {},
