...
describe("mock provider field repair behavior", () => {
  ...
  it("surfaces supplied advisory candidates exactly before local heuristics", async () => {
    const record: MainRunFieldRepairRecordInput = {
      recordId: "record-3",
      sourceText: "PING G425 shaft firm regular flex",
      missingFields: ["shaftFlex"],
      confidence: 0.7,
      selectionReason: {
        lowConfidence: true,
      ...
      advisoryCandidates: [
        {
          candidateId: "prior-review:learning-event-3:shaftFlex",
          sourceType: "PRIOR_REVIEW",
          sourceEvidenceId: "record-3:prior-review",
          sourceReferenceId: "learning-event-3",
          suggestion: {
            recordId: "record-3",
            fieldName: "shaftFlex",
      ...
      evidence: [
        ...
        {
          evidenceId: "record-3:prior-review",
        ...
        recordOutcomes: [
          {
            outcomeType: "REPAIR_SUGGESTED",
            recordId: "record-3",
            evidenceIds: expect.arrayContaining(["record-3:prior-review"]),
            suggestions: [
              {
                recordId: "record-3",
                fieldName: "shaftFlex",
    ...
    expect(validation.output?.suggestions).toHaveLength(1);
    expect(validation.output?.suggestions).not.toEqual(
        ...
        records: [
          {
            recordId: "record-1",
            sourceText:
              "unknown maybe 5w shaft unknown condition unclear value pending review",
            missingFields: ["shaftFlex", "conditionGrade", "tradeInValue"],
            confidence: 0.42,
            selectionReason: {
              lowConfidence: true,
